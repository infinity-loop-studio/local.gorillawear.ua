...
abstract class jshopUserShopBase extends JTable{
    ...
    function __construct(&$_db){
		...
		$obj = $this;
		JDispatcher::getInstance()->trigger('onConstruct'.ucfirst(get_class($this)), array(&$obj));
    }
	...
	function checkData($type, $check_exist_email){
		...
		$obj = $this;
        JDispatcher::getInstance()->trigger('onBeforeCheck'.ucfirst(get_class($this)), array(&$obj, &$type, &$config_fields, &$type2, &$return));
		...
		if ($type == "register"){
			...
			if ($this->u_name!=''){
				...
				$obj = $this;
                JDispatcher::getInstance()->trigger('onBeforeCheckUserNameExistJshopUserShop', array(&$obj, &$type, &$config_fields, &$type2, &$query));
		...
		if ($this->email!='' && $check_exist_email){
			...
			$obj = $this;
            JDispatcher::getInstance()->trigger('onBeforeCheckUserEmailExistJshopUserShop', array(&$obj, &$type, &$config_fields, &$type2, &$query));
			$this->_db->setQuery($query);			
